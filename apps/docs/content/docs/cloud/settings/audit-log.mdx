...
| `task` | `task.skill.create`, `task.skill.publish`, `task.skill.update`, `task.skill.unpublish` |
...
Use the resource filter to choose one of the resource types above. The actor filter is a single choice list of actors with their captured labels and entry counts. The page reads the selected resource from the URL and validates it before loading results.
