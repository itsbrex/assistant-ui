...
description: Telemetry endpoints, trace links, the page that draws a conversation as shown, and the SDK clients sending data to a project.
---

**Settings › Telemetry** connects the browser and server endpoints to the project, optionally links stored trace IDs to your trace backend, names the page of your app that draws a conversation as your users saw it, and shows which SDK versions still send requests. It is the page to check when telemetry reaches the cloud but a trace or client row is missing.
...
**Renderer URL** names a page of your app that mounts `CloudRendererHost`. Once it is set, a thread's Conversation offers **As shown** next to **Structured**, drawn by that page; see [Conversation views](/docs/cloud/dashboard/conversation-views). The page receives the whole conversation, attachments included, so point it only at a page you run.

The URL must be `https`, or `http` on `localhost` or `127.0.0.1` while you develop, with no credentials and no wildcard host, and at most 2,048 characters; anything else is refused with `Invalid renderer URL`. A fragment is dropped. Owners and admins save or clear it, and other members see it read only. The audit log records the write as `project.update` with `renderer_url` before and after.
...
| The Renderer URL is refused | It is not `https` (or `http` on localhost), carries credentials or a wildcard host, or is longer than 2,048 characters. | Use the page's `https` URL, or `http://localhost` while you develop. |
