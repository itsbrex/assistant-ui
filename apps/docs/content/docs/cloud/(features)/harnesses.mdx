...
`userId` is optional and defaults to the cloud's system user. `transport` accepts `"http"`, the default SSE transport, or `"websocket"`.
...
### Inspect a harness thread

A thread Instance id or Thread id opens its harness thread page at `/harnesses/{id}/threads/{thread_id}`. The separate **Mirrored thread** link opens the cloud thread.

The page's **Facts** rail shows **Thread ID**, **Workspace**, and **Backend**. **Backend** reads **Not pinned** when the thread has no pinned backend. Its **Conversation** tab shows the saved snapshot's main conversation, followed by **Subagent** sections. A saved state that is not a harness conversation falls back to JSON. **Raw** shows the saved state as JSON. When no state is saved, the page shows **No saved state**.
...
| Column | Meaning |
|---|---|
| Instance | A thread instance id opens its harness thread page. Its separate **Mirrored thread** link opens the cloud thread. |
| Kind | `thread` or `thread list`. |
| Connections | Current client connections. |
| Tasks | Current tasks. |
| Idle | How long the instance has been idle, such as `3 m 5 s`. |
...
| Column | Meaning |
|---|---|
| Thread | Copyable thread id that opens its harness thread page. |
