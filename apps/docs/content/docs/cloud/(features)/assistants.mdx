...
| **Model** | None | A model listed by the selected provider | Stores the provider model name. |
...
An Intelligence task can use an assistant as its skill. Linking one stores the assistant id on the task and records `task.skill.create` in the audit log.

## From your code

Run an assistant from your server. The in app runtime hook for assistants, `useCloudRuntime` in `@assistant-ui/react-data-stream`, is not ready: it posts a placeholder thread id, so the run answers `404`.
