...
Run an assistant from your server. The in app runtime hook for assistants, `useCloudRuntime` in `@assistant-ui/react-data-stream`, is not ready: it posts the conversation as language model messages, which the run endpoint rejects once a tool has run.
