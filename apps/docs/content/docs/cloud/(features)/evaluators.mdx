...
**Evaluator verdicts** in [Intelligence](/docs/cloud/intelligence) summarizes verdicts from the last 28 days. Its summaries are grouped by the rule's data type:
...
| The summary is lower than expected | **Evaluator verdicts** counts verdicts from the last 28 days, and the sampling rate selects only part of new conversations. | Check the rule's sampling rate and use the Intelligence, thread, or run score views to inspect the matching verdicts. |
