...
| Model | Empty | Optional model id from 1 to 255 characters. | Must be paired with a provider. A model without a provider is refused with `Select a provider for the evaluation model`. |
