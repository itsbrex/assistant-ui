...
A **message** is a node in the thread's tree. `parent_id` names the message it follows, so an edit or a regeneration starts a new branch under the same parent, and the cloud computes each message's `height` from its parent. `format` names the shape of `content`: `aui/v0` is what the local runtime stores, `ai-sdk/v6` what the AI SDK runtime stores, and a read may ask for a conversion between them. A message may also carry an `external_id` of yours, which makes storing it idempotent; `cloud.threads.messages.create` accepts one, and runtimes whose backend keeps the transcript use their backend's message ids for it. [Threads](/docs/cloud/threads) and [Messages](/docs/cloud/messages) go through both objects field by field.
...
The local runtime follows the same sequence with `aui/v0` messages. When `cloud` is passed, LangGraph, LangChain and ADK copy their settled messages after a run; their backend remains the transcript source and they do not send browser run reports. `telemetry: { messages: false }` keeps those copies out. A server with an API key makes the same calls itself, minus the token routes. [Which runtime](/docs/cloud/runtimes) lists what each integration stores and reports.
