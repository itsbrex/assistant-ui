...
|---|---|---|
| Threads | [Threads](/docs/cloud/dashboard/threads) | `from`, `to`, `search`, `created_by`, `satisfaction`, `signal`, `topic`, `task`, `question`, `junk`, `sentiment`, `resolved`, `language`, `feedback` (`negative`, `positive`, `any`), and `review` (`unreviewed`, `reviewed`, `good`, `needs_work`, `unclear`). |
