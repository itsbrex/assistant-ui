...
**Tasks** groups repeated requests. Its consistency reading is **Consistent**, **Mixed**, or **Inconsistent**. Open a task to inspect it in the current range. The **Tasks** table carries a **Thumbs down rate** that opens Threads with `task` set and `feedback=negative`. **View all** on Topics opens `/intelligence/topics`, and **View all** on Tasks opens `/intelligence/tasks`.
...
**Evaluator verdicts** shows the evaluator results alongside the analysis. With no evaluator rules, it reads **No evaluators yet** and offers **Configure evaluators**. A configured rule with no results reads **No verdicts yet**.
...
| Evaluator verdicts are empty | The project has no evaluator rules, or a configured rule has no verdicts in the last 28 days. | With **No evaluators yet**, select **Configure evaluators** to add a rule. A rule that reads **No verdicts yet** is judged only on the new conversations it samples: check in **Settings › Evaluators** that it is enabled and that its sampling rate lets conversations through, then wait for new conversations. |
