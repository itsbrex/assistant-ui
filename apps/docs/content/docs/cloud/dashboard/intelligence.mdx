...
**Tasks** groups repeated requests. Its consistency reading is **Consistent**, **Mixed**, or **Inconsistent**. Open a task to inspect it in the current range. The task page is the one place that offers **Create skill**: it links that task to an assistant, so the project can retain the association.
...
Opening a topic uses the active date range to show that topic's reading and its related Threads. Opening a task does the same for the task, its consistency, and its linked skill when one exists. The task's **Create skill** action connects the task to an assistant; it does not create a second task category.
...
| A task has no skill | A task and an assistant skill are linked only after **Create skill** is used on that task. | Open the task and create the link. |
