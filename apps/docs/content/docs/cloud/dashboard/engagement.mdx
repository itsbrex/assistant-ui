...
The six tiles come first. The **Feedback** block follows. It shows rated threads, thumbs up, thumbs down, positive rate, thumbs over time, and the most recent thumbs down with their comments.
...
**When people use it** is a weekday by hour heatmap. It counts runs by weekday and UTC hour, so patterns can be read by day of week and hour rather than from one daily total.
