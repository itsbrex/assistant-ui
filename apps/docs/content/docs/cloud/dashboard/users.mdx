...
| Last active | The user's most recent activity. |
| Threads | Threads attributed to the user in the range. |
| Runs | Runs attributed to the user in the range. |
| Tokens | Tokens attributed to those runs. |
| Cost | Cost attributed to those runs. |

**User**, **Runs**, **Tokens** and **Cost** sort. **Last active** and **Threads** do not. The page opens sorted by Runs and shows 50 people per page, with 25 and 100 as the other page sizes. Search matches user ids and display names.
...
Select a user to open their page. Its tiles, Models, Threads and Runs sections read the last 30 days. The dashboard range picker does not change that window. The header shows first seen and last active, and Activity is a 52 week calendar of daily runs over the last 365 days.
...
| Activity | A 52 week calendar of daily runs over the last 365 days. |
