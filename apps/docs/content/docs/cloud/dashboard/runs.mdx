...
The main tabs are **Trace**, **Conversation**, **Scores**, and **Raw**. **Conversation** shows the user and assistant turn. When a run has no thread, it shows the conversation read from the trace as the model saw it at the last step. **Scores** is a table with an **Annotate** action for runs attached to a thread. **Raw** shows the run as JSON.

**Usage** is a share bar for **Fresh input**, **Cached input**, **Output**, and **Reasoning**, followed by **Cost**, **Duration**, and, when a run has more than one generation, one row for each step. **Attributes** is the run's attributes ledger.

The trace waterfall gives every stored span a row. Select a span to open its rail, which has **Input**, **Output**, **Details**, and **Raw** tabs. When they contain OpenTelemetry messages, **Input** and **Output** show them by role. **Input** offers **Show N earlier messages** when earlier messages are hidden. A tool input that contains a generative UI tree is drawn as generative UI. **Details** shows **Kind**, **Tool**, **Source**, **Model**, **Step**, **Finish reason**, **Tool call id**, **Offset**, **Duration**, **Status**, **Status code**, **Error type**, **Started**, **Ended**, **Input tokens**, **Output tokens** and **Cost**. A missing or zero span cost reads *Not priced*. **Raw** shows the span as JSON. In the waterfall, Up and Down select a span, Right and Left expand and collapse it, and Esc clears the selection.
...
| Cost reads `—`. | The run has no recorded `cost_usd` and its model has no matching catalog price or project price override. | Add a matching price override in [Model prices](/docs/cloud/settings/model-prices), or include cost in the run report. |
