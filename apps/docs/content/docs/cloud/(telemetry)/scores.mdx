...
The thread component installed with `npx assistant-ui add thread` shows thumbs up and thumbs down on assistant messages whenever its runtime has a feedback adapter. Assistant Cloud runtimes provide that adapter. It does not collect a comment. To collect one, add [`FeedbackDialog`](/elements/feedback-dialog) with `npx assistant-ui add feedback-dialog` and pass its note to `submitFeedback` as `comment`.
