...
The hook gives the runtime Cloud threads and the history, attachment and feedback adapters as defaults; an adapter the wrapped runtime supplies itself is kept. A runtime built on `useExternalStoreRuntime` stores a copy of its settled messages through that history adapter, as [LangGraph, LangChain and ADK](/docs/cloud/langgraph) describes. A direct adapter is available when you need the list without the wrapper runtime.
...
| `create` | `undefined` | Runs before the Cloud thread is created and receives the thread list item's `id`. Its `externalId` is saved as the Cloud thread's `external_id`. |
...
| `create` | `undefined` | Resolves an optional external id before `cloud.threads.create`. It receives the thread list item's `id`. |
| `delete` | `undefined` | Runs before `cloud.threads.delete`. |
| `upsert` | `undefined` | Creates with `upsert`, so a create retried after a lost response reuses the Cloud thread that already has the returned external id. Set it when that id names exactly one conversation. |
...
| `initialize(threadId)` | Awaits `create(threadId)`, then calls `cloud.threads.create` with the current time and the returned `external_id`. It returns the Cloud `remoteId` and the external id. |
...
AG-UI and A2A do not create a Cloud adapter themselves. Place either per-thread runtime inside `useCloudThreadListRuntime` to give it Cloud threads, message history, attachments, feedback, engagement events and reports read from stored assistant messages. AG-UI stores its final agent state in `metadata.unstable_state`, activity values as `agui-activity/<type>` data parts, and A2UI `present` calls with `artifact.a2ui`. A2A stores the same A2UI calls and its artifacts in `metadata.custom.a2a.artifacts`, without bytes from inline files. See [Stored message formats](/docs/cloud/formats).
