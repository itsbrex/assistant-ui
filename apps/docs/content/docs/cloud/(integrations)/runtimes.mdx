...
| [LangGraph, LangChain, ADK](/docs/cloud/langgraph) | yes | `aui/v0`, a copy of settled messages | at the first message | none; export traces | engagement and feedback; attachments through the runtime's own adapter |
| [OpenCode](/docs/runtimes/opencode/quickstart#store-threads-in-assistant-cloud), [Pi](https://github.com/assistant-ui/assistant-ui/tree/main/packages/react-pi#assistant-cloud), [assistant transport](/docs/runtimes/custom/assistant-transport#adapter-support) | yes | `aui/v0`, a copy of settled messages | at the first message | none; export traces | engagement and feedback; attachments through the runtime's own adapter |
...
- **Copied messages.** LangGraph, LangChain, ADK and any runtime built on `useExternalStoreRuntime` keep the transcript on their backend. When a run settles, the runtime stores a copy of the messages that are new or changed as `aui/v0`, keyed by the backend's message ids, with any tool interaction a user recorded kept on its tool call in the copy. The dashboard shows these threads like any other and feedback can rate their messages; the runtime never reads the copy back. `telemetry: { messages: false }` on the cloud client keeps the copy out, and tool interactions are then rejected; `telemetry: false` turns it off together with run reports and events.
