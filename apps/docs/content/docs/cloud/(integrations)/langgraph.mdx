...
| Messages | When a run settles, the runtime stores a copy of new and changed eligible messages as `aui/v0`, keyed by stable backend message ids, so the dashboard shows the transcript. A message without a backend id is not copied. Earlier messages of a conversation are copied the first time it continues after the upgrade. The runtime never reads the copy back; your backend stays the source. |
...
| The cloud message id | `[assistant-ui] Skipping feedback for message <id>: no cloud message id is mapped.` | The message has not been copied yet: it is still streaming, has no backend id, the copy failed, or `telemetry: { messages: false }` is set. The copy is written when the run settles. |
