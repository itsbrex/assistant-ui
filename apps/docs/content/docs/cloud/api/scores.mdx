...
`source` and `comment` are returned by `GET /v1/projects/scores`, not by `POST /v1/scores`.
...
| `404` | Plain text: `Thread not found` | The target thread is not visible in the caller's scope. |
| `404` | Plain text: `Run not found` | The target run is not visible in the caller's scope. |
