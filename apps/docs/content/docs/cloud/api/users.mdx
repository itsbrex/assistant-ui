...
6. It deletes the user's `workspaces`, `project_users` and `refresh_tokens` rows.
7. It recomputes `thumbs_up` and `thumbs_down` for the threads where the user left feedback, then deletes the user's authored `scores`, `client_events` and `client_events_daily` rows.
8. It sets `runs.created_by` to `usr_deleted`. The number of rows changed by this step is returned as `runs`.
9. It folds the user's daily run rollups into the `usr_deleted` key, then deletes the user's `runs_daily` rows.
...
| `403` | Plain text: `Project-wide user deletion requires API key authentication` | The credential is not an API key, or API key access failed. |
