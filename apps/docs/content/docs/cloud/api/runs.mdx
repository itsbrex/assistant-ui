...
| `400` | Plain text: `message_id must belong to thread_id` | `message_id` is not a message in `thread_id`. |
| `404` | Plain text: `Thread not found` | The thread is not in the caller's workspace. |
...
| `trace_id` | string | no | 1 to 48 characters. Used only for `system/thread_title`; assistant runs create their own trace id. |
...
| `400` | Plain text: `Invalid messages format: <Zod error message>` | A message does not have an allowed role, content array, or typed content item. |
| `400` | Plain text: `Invalid response format` | A normal assistant run did not request `vercel-ai-data-stream/v1`. |
| `400` | Plain text: `Provider base URL must be a valid URL`, `Provider base URL must use HTTPS`, `Provider base URL must use HTTP or HTTPS` or `Provider base URL must not target a private or loopback address` | The selected assistant's provider base URL was refused. |
| `404` | Plain text: `Thread not found` | The thread is not in the caller's workspace. |
| `404` | Plain text: `Assistant not found` | No selected assistant is available to the caller. |
| `401` or `403` | An authentication error. | The caller was not accepted. See [Conventions](/docs/cloud/api). |
