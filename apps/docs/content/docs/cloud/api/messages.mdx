...
| `content` | object | The format's message payload. Its serialized JSON is at most 1,048,576 characters. |
...
| `content` | object | yes | The serialized JSON is at most 1,048,576 characters. Its `role` controls user activity tracking and the title trigger. |
...
| `400` | `{ "success": false, "error": … }` | A required field is missing, a field is invalid, content exceeds 1,048,576 serialized characters, or an unknown field was sent. |
...
| `404` | Plain text: `Project not found` | The project no longer exists. |
...
| `content` | object | yes | Replaces the content. Its serialized JSON is at most 1,048,576 characters. |
...
Any message in the thread can be updated, including one that later messages follow, and it keeps its parent and its children. The write records `updated_by` but does not advance the thread's `last_message_at`.
