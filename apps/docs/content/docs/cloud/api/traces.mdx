...
The traces endpoint receives OTLP/HTTP exports from server side instrumentation and turns GenAI spans into runs. It accepts an API key on `https://backend.assistant-api.com`; `Aui-Workspace-Id` is optional on this route, unlike the other API key routes. The general host and credential rules are in [Conventions](/docs/cloud/api), and shared errors are in [Errors](/docs/cloud/api#errors).
...
| `Content-Length` | header | no | A declared body larger than 4,194,304 bytes is refused. A gzip or deflate body is decompressed before streamed content is counted against the same cap. |
| Request body | OTLP export | yes | At most 4,194,304 bytes, or 4 MB, after gzip or deflate decompression. |
...
| `401` | Plain text: `Authorization header is missing` | `Authorization` is missing. |
| `403` | Plain text: `This endpoint requires an API key` | The caller supplied a JWT instead of an API key. |
