...
| `403` | Plain text: `Anonymous sessions cannot claim threads` | The caller is an anonymous identity. |
| `403` | Plain text: `Invalid refresh token format` | The token does not start with `refresh_0`. |
| `403` | Plain text: `Invalid refresh token` | No live anonymous token of this project matches. |
