...
The Assistant Cloud history adapter has `update`, so a task paused for `input_required` or `auth_required` writes its assistant question to history. Other history adapters record the question only when they expose `update`; an append-only adapter stores only the settled assistant message.
