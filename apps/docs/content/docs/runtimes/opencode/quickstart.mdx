...
Pass `cloud` to back the thread list with [Assistant Cloud](/docs/cloud) instead of the OpenCode server's session list. Each cloud thread maps to an OpenCode session through its external id: a new thread creates a session, deleting the thread deletes its session when the session still exists, and the dashboard shows the conversation. The thread list opens cloud threads, so `initialSessionId` is not used with `cloud`. A cloud thread that no OpenCode runtime created has no session: it opens empty, and sending in it rejects. Renaming and archiving change the cloud thread only; the OpenCode session keeps its own title.
