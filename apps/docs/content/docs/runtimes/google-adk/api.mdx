...
Pass a `cloud` instance to use Assistant Cloud for the thread list and a dashboard copy of settled messages. Thread history loads through the `load` function from the ADK session, not from Assistant Cloud:
