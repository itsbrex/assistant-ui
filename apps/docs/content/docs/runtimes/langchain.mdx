...
| --- | --- | --- |
| `cloud` | `AssistantCloud` | Optional. Uses Assistant Cloud for the thread list and a copy of settled messages for the dashboard. |
| `create` | `() => Promise<{ externalId: string \| undefined }>` | Optional. Creates a thread in your backend. |
| `delete` | `(threadId: string) => Promise<void>` | Optional. When Cloud owns the thread list, called before a thread is deleted and given its Cloud thread id; read that thread's `external_id` to delete the backend thread, as [LangGraph](/docs/cloud/langgraph) shows. |
| `onThreadIdChange` | `(threadId: string \| undefined) => void` | Optional. Called when the active thread's canonical ID changes. |
| `unstable_threadListAdapter` | `RemoteThreadListAdapter` | Optional. Replaces the Cloud thread list adapter. |
...
Pass an `AssistantCloud` instance to persist the thread list across sessions. `useStream({ threadId: externalId })` loads state from the LangGraph server, not from Assistant Cloud. The runtime copies settled messages to Assistant Cloud for the dashboard; set `telemetry: { messages: false }` on the cloud client to disable that copy.
