...
A turn whose tool call opened its own record comes back as one message. A tool call that carries no `parentMessageId` has no record to join, so the run opens one keyed by the call id, and the prose that answers it lands in a record of its own; a turn that reasoned, called a tool, then answered sits on the wire as a `reasoning` record and two assistant records, and one that called two tools in parallel sits there as three. An assistant record whose own id is one of the calls it carries is that opened container rather than a boundary an agent drew, so `fromAgUiMessages` folds what follows it, another container for a parallel call or the parts of the answer, onto it as parts of one message, under the id the agent addressed its prose to. That turn leaves on the next run as a single assistant record carrying both the prose and the calls, followed by the tool results, so the text is restored ahead of the calls by the position limit above.
...
The fold is best effort, because nothing on the wire carries run identity. The fingerprint says a record is not a boundary an agent drew; it does not say which side of the container the turn began on, so two shapes stay imperfect. Prose the run emitted before its tool call keeps a message of its own, and the call folds forward into the record that answers it, so that turn comes back as two messages split between the prose and the call rather than the one message the live run rendered. An assistant record that a later run produced after a parentless container, with no user message between the two, is folded into that container's turn.
