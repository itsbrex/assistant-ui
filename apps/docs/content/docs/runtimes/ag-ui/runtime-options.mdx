...
Neither is the message count of a turn. A run opens a new assistant record for a tool call that carries no `parentMessageId`, and another for the prose that follows it, so a turn that reasoned, called a tool, then answered sits on the wire as a `reasoning` record and two assistant records. The reasoning rejoins the first of them; the two assistant records stay two messages, because nothing on the wire binds them to one run. A live run renders that same turn as one message, so a thread reloaded from a snapshot can carry more messages than the session that produced it.
