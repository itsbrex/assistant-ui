...
If the user sends another message instead of answering, the pause ends there, because a run only resumes from the last message. Each open approval records `approval.resolution: "cancelled"` and the paused message settles as `incomplete` with reason `cancelled`; a later answer to that approval is rejected, and a result added to it later is stored without invoking your adapter again. The next run still receives those calls without results, and providers reject a tool call that no result answers, so your adapter must supply one when it converts the history; `toGenericMessages` from `assistant-stream` reports each as not completed.
