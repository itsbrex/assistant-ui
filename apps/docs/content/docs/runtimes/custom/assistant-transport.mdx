...
| Assistant Cloud | `cloud`, or `NEXT_PUBLIC_ASSISTANT_BASE_URL` without it: the cloud backs the thread list, every request's `threadId` is the cloud thread id, feedback is stored, and settled messages are copied for the dashboard |
