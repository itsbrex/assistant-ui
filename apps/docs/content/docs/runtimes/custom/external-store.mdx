...
| `onVoiceTranscript` | Persist finalized voice transcripts. Convert the `ThreadMessage` to the host message type when one is used and retain its id. |
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      description:
        "Receives each finalized voice transcript message; when your host uses its own message type, convert the `ThreadMessage` while keeping its id, then append it so it persists. Without it, transcripts stay for the session only.",
    },
    {
