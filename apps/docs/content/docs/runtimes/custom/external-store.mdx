...
<Callout type="info">
**Real-world example:** Anthropic's official [Claude Managed Agents quickstart](https://github.com/anthropics/claude-quickstarts/tree/main/managed-agents/assistant-ui) maps Managed Agents sessions to assistant-ui threads with `useExternalStoreRuntime` and `useRemoteThreadListRuntime`.
</Callout>
