...
| `onAddToolResult` | Client-side tool result handoff. Any of `onAddToolResult`, `onResumeToolCall`, and `onRespondToToolApproval` turns on `s.thread.capabilities.answerToolCall`, which the tool fallback's answer controls need |
