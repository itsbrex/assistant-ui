...
- **Threads** (single, cloud, custom database) work via a shared `RemoteThreadListAdapter` for `LocalRuntime`-based runtimes. A standalone `ExternalStoreRuntime` uses an `ExternalStoreThreadListAdapter`; runtime adapters that receive `cloud` use Assistant Cloud's remote thread list adapter. See [threads](/docs/runtimes/concepts/threads).
