...
| --- | --- | --- | --- |
| AssistantCloud | LocalRuntime and runtime adapters that accept `cloud` | assistant-cloud | You want the thread list managed; auth and sync handled |
...
`AssistantCloud` is the managed multi-thread service. Pass an instance to `useLocalRuntime` or a runtime adapter that accepts `cloud`, and the thread list, sync and titles are handled for you. Where messages are stored depends on the runtime: `useLocalRuntime` and the AI SDK runtime, plus AG-UI and A2A through `useCloudThreadListRuntime`, store and load them in the cloud, while LangGraph, LangChain, ADK and other runtimes whose backend keeps the transcript load it from that backend and store a copy in the cloud for the dashboard, as [Runtimes](/docs/cloud/runtimes) lists.
...
const runtime = useStreamRuntime({ cloud /* LangGraph server options */ });
const runtime = useOpenCodeRuntime({ cloud /* OpenCode options */ });
const runtime = usePiRuntime({ cloud /* Pi options */ });
const runtime = useAssistantTransportRuntime({ cloud /* transport options */ });
