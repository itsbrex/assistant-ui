...
`load` runs when a thread opens. `append` runs after each message completes. `update` rewrites a stored message in place, which lets the runtime store a run paused for tool approval and a tool result that arrives after its message settled; without it, a paused run is stored only once it ends, for example when a later turn cancels it.
