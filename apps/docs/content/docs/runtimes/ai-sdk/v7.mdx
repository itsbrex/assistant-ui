...
While a handler is set, an approval's `display`, `allowFreeform` and `options` reach the renderer, so select and text requests render their own controls. Those fields come from messages your app loads or sets. An AI SDK stream cannot produce them, because the `approvalDescriptor` of a `tool-approval-request` chunk arrives as `approval.descriptor`, which is passed through unchanged.
