...
The answer applies to the approval as soon as the handler starts, and a second response to the same request is rejected while the runtime that received it stays mounted. It is held by that runtime and never written into the `useChat` messages, so `sendAutomaticallyWhen` cannot send it to your chat route. With any history adapter, including the Assistant Cloud adapter, it is stored under `__aui_toolApprovalResponses` and restored when history loads. Without a history adapter, a reload or a runtime mounted again over the same chat (for example switching away from an in-memory thread and back) shows the request open until the chat records its resolution, so bring the resumed run back into the chat (for example with `resumeStream`) and have your endpoint refuse a second resume. If the handler throws, the request reopens and `respondToApproval` rejects with the handler's error.
