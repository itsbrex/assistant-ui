...
With Assistant Cloud, add `cloud` to the `useChatRuntime` call of either pattern and the app gets the thread list, message persistence, automatic titles, run reports and engagement events; a `messageMetadata` callback on the route fills in the model and usage of each run the same way as on any AI SDK route. See [AI SDK + assistant-ui](/docs/cloud/ai-sdk-assistant-ui).
