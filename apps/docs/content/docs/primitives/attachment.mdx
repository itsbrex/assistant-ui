...
- **Messages**: via `MessagePrimitive.Attachments`. These are sent attachments, displayed read-only (the iterator renders attachments for user messages). While the message is the composer's submission, its attachments are still being prepared, so the iterator hands the render function an `Attachment` rather than a `CompleteAttachment`: read `attachment.status` before `attachment.content`.
