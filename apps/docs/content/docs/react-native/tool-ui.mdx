...
The fallback receives the whole tool call part. Pass it through the thread's `components` prop. The installed fallback also answers approval gates, interrupts and approval questions, and a replacement takes that job over, so this one renders `ToolFallbackApproval` from the installed fallback while a call waits, and again once its approval settles, when the same component shows a receipt of the decision. Keep the replacement in its own file: `components/assistant-ui/elements/tool-fallback.tsx` is the installed fallback, which the bound task card imports.
  ...
  return (
    <View className="gap-2">
      <ToolTimeline
        ...
        stats={[]}
      />
      {(waiting || settled) && <ToolFallbackApproval {...part} />}
    </View>
  );
};
```
