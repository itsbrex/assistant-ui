...
The codemod follows recognized client bindings rather than every variable with the same name. Clients received through properties, such as `const { aui } = props` or `const aui = ctx.aui`, may need manual updates. Review the diff and run your project's typecheck after migrating.
