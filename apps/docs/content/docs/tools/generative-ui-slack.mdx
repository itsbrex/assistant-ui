...
| `Fact` | Merged into one `section`'s `fields`, as `*label*` then value | Consecutive facts merge; every 10 fields start a new section; `delta` appends as the browser's directional chip in parentheses |
| `Table` | `data_table` block, first row as header | Column formats produce the same formatted text as the browser; sorting and alignment are browser-only; rows are padded to a uniform width; a column without a string label keeps its position with an empty header and warns |
...
| `CheckboxGroup` | `checkboxes` element | An option without a string label and value is dropped with a warning; Slack allows 10 options |
| `Checkbox` | `checkboxes` element with a single option | The `switch` variant uses the same element |
| `Slider` | Its own `input` block with `number_input` | `min`, `max`, and `defaultValue` map to number bounds and initial value; fractional steps allow decimal input |
...
`RadioGroup` and `CheckboxGroup` option descriptions map to Slack option `description` text.

Slack has no undo surface, so a Button with `undoable` fires immediately.
...
<Callout type="warn">
Only buttons carry `value`. `Select`, `Input`, `Slider`, `DatePicker`, `Checkbox`, `RadioGroup`, and `CheckboxGroup` emit `action_id` alone, so any extra keys on their `$action` are dropped. Keep those controls' actions to a bare `type`, or put the payload on a button that submits alongside them. A `{ "$field": name }` reference in a button's payload becomes its `fallback`, or is dropped when it has none, with a `fallback` warning, because a button click carries no other control's value.
</Callout>
...
| `number_input` | The entered number |
...
The round trip is faithful on the plain building blocks (text, images, facts, controls, tables, simple cards) and documented-lossy elsewhere: context elements all return as `Caption`, so the `Badge` distinction is gone, a `CheckboxGroup` with a single option returns as a `Checkbox`, button styles beyond `primary` and `danger` are dropped, an alert's title and description come back merged into the description, and card layouts flatten to the fields the `card` block carries. Card footers are re-derived from button style, with the primary-styled button becoming `confirm`; when style cannot decide it, position does, and that emits a `fallback` warning.
