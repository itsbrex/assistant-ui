...
assistant-ui uses "generative UI" for more than one thing. Pick the row that matches what you are building:
...
| **Tool UI** | toolkit `render` | A widget tied to a tool you already know about (forms, pickers, charts) |
| **Generative UI primitive** | `MessagePrimitive.GenerativeUI` + allowlist | Existing integrations that emit `generative-ui` message parts ([legacy](/docs/tools/generative-ui-legacy)) |
| **LangGraph data UI** | `makeAssistantDataUI` + `ui_message` | LangGraph agents emitting UI on the LangGraph stream |
...
To restyle a single component instead, target its attribute:
