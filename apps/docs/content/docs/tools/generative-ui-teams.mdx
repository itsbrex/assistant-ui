...
| `CheckboxGroup` | `Input.ChoiceSet`, expanded, with `isMultiSelect` | Its value is the checked values joined by commas, so a value that contains a comma cannot be told apart |
