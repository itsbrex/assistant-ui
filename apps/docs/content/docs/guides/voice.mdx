...
Of the shipped runtimes, the AI SDK runtime implements `onVoiceTranscript`: it appends each transcript to the `useChat` messages, where a configured history adapter stores it like a typed turn. `eve`, `react-a2a`, `react-ag-ui`, `react-google-adk`, `react-langchain`, `react-langgraph`, `react-opencode`, and `react-pi` accept a voice adapter but do not implement it, so on those runtimes transcripts render for the session only, never reach the provider thread, a history adapter, or the model, and are dropped on disconnect.
