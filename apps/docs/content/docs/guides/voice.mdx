...
While a session is connected the composer cannot send: `canSend` is false and `thread.append` rejects with "Cannot send a text message while a voice session is connected". Edits, reloads, and runs are rejected while connected as well on both runtimes. Tool results and approvals are rejected on the local runtime; an external store forwards them to the host, which decides. Hanging up commits the sentence the assistant was speaking. A queue on the local runtime holds its items while a session is connected and resumes after disconnect; a queue owned by an external store host is the host's to pause. Disconnect first; the session is the only input for its duration. The rule holds in both directions: `connectVoice` rejects while a run is in progress or paused on a pending tool action, and the kit disables the connect button until the run settles.
