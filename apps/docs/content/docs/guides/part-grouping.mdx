...
The render function receives both group nodes and leaf parts through `{ part, children }`. Only group cases should render `children`. For a leaf, returning `null` renders its registered tool or data UI when one exists. Return an empty fragment (`<></>`) when you want to suppress a registered UI, or return your own component for a custom leaf renderer. Text and other unregistered leaves render nothing when `null` is returned.
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      description:
        "Render function called for group nodes and leaf parts. Group nodes expose children. Returning null for a tool or data leaf renders its registered UI; an empty fragment suppresses it. Other leaves render nothing when null is returned.",
      required: true,
    },
  ]}
/>
