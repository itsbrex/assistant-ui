...
An agent at index `i` is done when `i < completedCount`; its bar width comes from `progress[i]`, clamped into `0…100` with a missing or `NaN` entry read as `0`. The summary card, when `showSummary` is true, always shows the spinner treatment and a fixed bar; this version has no way to mark the summary itself done, only to show or hide it. The root carries a minimum height so the layout does not jump as agents complete and the summary card fades in.
...
| `progress` | `readonly number[]` | required | Bar width per agent, index-aligned with `agents`. Out-of-range or `NaN` entries read as `0`. |
