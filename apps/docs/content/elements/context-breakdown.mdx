...
Headroom is not a segment you pass; it is `limit` minus the sum of every segment's `tokens`, floored at zero, and it never gets a bar slice of its own, only a legend row. A zero `limit` is treated as zero pressure rather than dividing by it, so the header stays plain instead of showing a red or broken percentage. Every segment supplies its own `tint`, a Tailwind background class shared verbatim between the bar slice and the legend dot, so the two never disagree.
