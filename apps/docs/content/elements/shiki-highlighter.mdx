...
Pass one-based line numbers to add the `highlighted` class to each matching Shiki line. When the result sits in the kit's `CodeBlock`, that class receives its existing highlighted-row treatment.
...
Lines are marked after Shiki resolves. While `streaming` is true the plain fallback remains unmarked.
...
| `highlightLines` | `readonly number[]` | | One-based lines to mark with the `highlighted` class after Shiki resolves. |
