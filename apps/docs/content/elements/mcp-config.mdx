...
`AuthFields` renders a labeled bearer token input or OAuth scopes input depending on the selected `AuthSelect` value by default; pass `children` to render your own inputs for the current auth type instead.
