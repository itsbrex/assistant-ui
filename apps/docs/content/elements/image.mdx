...
function ToolImage({ imageUrl, source }: ImageToolResult) {
  return (
    <Image.Root>
      <Image.Zoom src={imageUrl}>
        <Image.Preview src={imageUrl} ratio="16:9" fit="cover" />
      </Image.Zoom>
      <Image.Source {...source} />
    </Image.Root>
  );
...
`Image.Source` uses `label`, or the URL host when no label is supplied. A safe URL makes the label a new-tab link. Give it `iconUrl` to show a 16px favicon; without one, it uses the source initial.
...
| `Image.Preview` | `div` + `img` | Loads `src`, shows a placeholder until it does, an error icon if it fails. `ratio` and `fit` control fixed framing. |
| `Image.Source` | `div` | Optional source attribution footer with an icon and label or host. |
...
`Image.Root`, `Image.Preview`, `Image.Source`, `Image.Zoom`, `Image.Filename`, `Image.Generating`, `Image.ContentFilterError`, and `Image.Actions` are exported individually for a custom layout, as shown in the examples above.

### ImagePreview

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `ratio` | `"auto" \| "1:1" \| "4:3" \| "16:9" \| "9:16"` | `"auto"` | Keeps the image's natural height, or creates a fixed aspect-ratio frame. |
| `fit` | `"cover" \| "contain"` | `"contain"` | Crops to fill a fixed frame, or keeps the full image visible. |

### ImageSource

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `label` | `string` | | Attribution label. Falls back to the URL host. |
| `url` | `string` | | Safe URLs make the label a new-tab link and provide the fallback host. |
| `iconUrl` | `string` | | 16px favicon URL. Without one, the source initial appears in a muted square. |
| `className` | `string` | | Merged onto the attribution footer. |

All other `div` props are forwarded to `Image.Source`. It renders nothing without a label or a displayable URL host.
