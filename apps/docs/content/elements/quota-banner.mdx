...
`left` is `limit` minus `used`, floored at zero, so going over quota reads as `"0 {unit} left"` rather than a negative number. The tight, amber state starts at 90% used (`used / limit >= 0.9`), coloring both the headline and the fill bar; a `limit` of `0` is treated as `0%` used rather than producing `NaN`. The upgrade button always renders, whether or not the account is close to its limit, and calls `onUpgrade` if you pass one, there's no default action if you don't.
