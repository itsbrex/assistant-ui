...
Voice turns are appended as ordinary messages, so the transcript is the same `s.thread.messages` any text thread uses, not a separate voice-only log (an external store host takes them through `onVoiceTranscript`, see the [voice guide](/docs/guides/voice)):
...
Select the array and derive in `useMemo`; a selector that maps to a fresh array re-renders on every store update.
