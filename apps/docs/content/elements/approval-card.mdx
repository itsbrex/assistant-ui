  ...
  run_command: {
    ...
    render: ({ args, approval, respondToApproval, result }) => (
      <ApprovalCard
        ...
        onAllowOnce={() => respondToApproval({ optionId: "once" })}
        onAlwaysAllow={() => respondToApproval({ optionId: "always" })}
        onDeny={() => respondToApproval({ optionId: "deny" })}
      />
    ),
  },
});
...
```tsx
onDeny={() =>
  respondToApproval({ optionId: "deny", reason: "not in this session" })
}
```
...
| `respondToApproval(response)` | `(response: ToolApprovalResponse) => void` | Sends the decision. Legal only while `approval.approved` is `undefined`. |
