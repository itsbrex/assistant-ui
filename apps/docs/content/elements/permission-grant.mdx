    ...
    <div className="flex flex-col gap-3.5 rounded-[20px] border p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {(approval.options ?? []).map((option) => (
          <button key={option.id} type="button" onClick={() => respondToApproval({ optionId: option.id })}>
...
{(approval.options ?? []).map((option) => (
  <div key={option.id} className="flex flex-col gap-1">
    <button type="button" onClick={() => respondToApproval({ optionId: option.id })}>
      {option.label ?? option.kind}
    </button>
...
if (confirming) {
  return (
    <div>
      ...
      <button onClick={() => respondToApproval({ optionId: confirming.id })}>Confirm</button>
...
```tsx
<button type="button" onClick={() => respondToApproval({ approved: true })}>
  Allow
</button>
<button type="button" onClick={() => respondToApproval({ approved: false })}>
  Deny
</button>
```
...
| `respondToApproval(response)` | `(response: ToolApprovalResponse) => void` | Answers the gate. Accepts `{ approved }`, `{ optionId }`, or both. Only valid while `approved` and `resolution` are both unset. |
