...
Progress is computed purely from `spokenIndex` divided by `words.length`, there's no separate progress prop, so the bar and the highlighted word always agree. Words before `spokenIndex` dim, the current word gets a highlight background, and the rest stay at full opacity, so `spokenIndex` alone determines all three states across the whole sentence.
