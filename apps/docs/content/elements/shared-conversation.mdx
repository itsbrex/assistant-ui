...
`ReadonlyThreadProvider` wraps children in a read-only thread. It reports itself disabled, so a composer inside renders disabled; thread mutations are ignored, and composer text, sending, adding attachments, and setting a quote have no effect. The normal thread primitives render it exactly like any other thread.
...
Every capability on this thread, `edit`, `delete`, `reload`, `cancel`, `answerToolCall`, and the rest, reports `false`, so composer input, regenerate, and branch controls that check `s.thread.capabilities` disable themselves and the tool fallback drops its approval controls while keeping the prompt, with no extra work on your part.
...
| Part | Renders | Notes |
|------|---------|-------|
| `ReadonlyThreadProvider` | children | Wraps `children` in a disabled thread scope built from `messages`. Thread mutations and composer text, send, addAttachment, and setQuote calls have no effect. |
...
| `s.thread.capabilities` | `RuntimeCapabilities` | Every flag (`edit`, `delete`, `reload`, `cancel`, `attachments`, `answerToolCall`, and the rest) is `false`. |
