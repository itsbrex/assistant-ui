...
Levels render in the order given; there is no reordering or grouping. If `selectedKey` does not match any `level.key`, the budget resolves to 0 and the progress fill stays collapsed at 0 percent rather than erroring. The fill's width is `spent` as a share of the matched level's `budget`, clamped between 0 and 100 percent, so an over-large `spent` value does not overflow the bar.
