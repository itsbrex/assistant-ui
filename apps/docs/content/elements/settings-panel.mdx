...
An OpenAI reasoning model accepts `temperature` only while its reasoning is off, so the route sets `reasoningEffort: "none"`. The AI SDK drops `temperature` for GPT-6 models at every effort, which is why this example uses GPT-5.6.
