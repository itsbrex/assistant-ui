...
Each file header includes a copy action by default. It copies that file as a unified diff, including its file headers and hunks. Set `copyable={false}` when the diff should not be copied.

```tsx
<DiffViewer patch={diffString} copyable={false} />
```

### Long Files

Use `maxCollapsedLines` to keep a long file compact. Files at or below the limit stay expanded, and each longer file can be expanded independently.

```tsx
<DiffViewer patch={diffString} maxCollapsedLines={24} />
```
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      type: "boolean",
      default: "true",
      description: "Show a file-header action that copies each file as a unified diff.",
    },
    {
      name: "maxCollapsedLines",
      type: "number",
      description: "Collapse a file when its rendered rows exceed this limit.",
    },
    {
      name: "className",
      type: "string",
