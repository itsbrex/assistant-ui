...
- **Size budgets gate.** `pnpm size:check` bundles every published entry with rolldown (all bare imports external, minified, gzip) and compares it with `size-budgets.json`. A move past max(2%, 256 B) in either direction fails; `pnpm size:update` after a full build rewrites the budgets and the diff rides in the PR as the reviewable claim.
