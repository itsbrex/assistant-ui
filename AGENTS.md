...
- Runtime-value coupling (the package imports upstream functions or classes: ai-sdk, react-opencode, react-mcp): the upstream lives in `dependencies` on a single caret major, never a `||` union. When a workspace package it installs peers the same upstream, the upstream is a required peer on that caret major instead, its floor raised only when the code requires a newer API, because pnpm keys each instance by the peers it resolves and a private copy splits the shared package in two once the host moves its own copy: ai-sdk takes `ai` this way for `assistant-cloud`, and `scripts/check-workspace-ranges.mjs` enforces it. An upstream major lands as one atomic PR that moves the package and every workspace consumer together; for the AI SDK this includes the docs vN page plus vN-legacy stub, the example rename, and the redirect. Users staying on the old major pin the last release of the previous line; there are no backports.
...
- **Wall-time and rendering lanes inform, never gate.** The Performance workflow benches head against base on one runner and traces headless-Chrome fixtures, then posts one sticky PR comment with a `machine-readable` JSON block; it runs once when a PR opens, reopens or leaves draft, once more on the autofix.ci commit while the PR has no report (autofix.ci cancels every run on the commit it fixes), and on every push only while the PR carries the `type/perf` label. The nightly workflow records main on the `perf-history` branch. A comparison runs only the benches that exercise a package whose built `dist` changed, plus at most three bench files on unchanged dists as controls that calibrate that run's noise floor, and runs nothing when no measured dist changed; a row is a verdict only when its bench exercises a changed dist. A `SLOWER` row on an untouched package is by construction noise, and a green comparison covers only the paths a bench exercises.
