...
`pnpm typecheck` runs the turbo `typecheck` task: `tsc --noEmit` in every workspace that is not a Next.js app (those are type checked by `next build`), with a preparation step first where a workspace needs one (`svelte-kit sync`, React Router typegen, `next typegen` for the route and image types a Next.js app's tests inherit), plus `tsc --noEmit -p tsconfig.test.json` wherever a workspace checks its tests in a second program. A Next.js app carries only that second program, since `next build` already covers its source. `.vue` and `.svelte` component bodies are outside the gate until a Vue or Svelte checker runs under the pinned TypeScript (#7560), and the two Nuxt workspaces stay out for the same reason. CI runs the task on the changed packages, so a type error in a test file fails the PR the same way one in `src` does.
